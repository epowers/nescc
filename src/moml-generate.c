...
static void prt_nc2momllib_header() {
...
static void prt_nc2momllib_component_ports(nesc_declaration c) {
  const char *ifname;
  void *ifentry;
  env_scanner scanifs;
...
bool generate_momllib(dd_list nesccomponents, const char *inputpathname) {
    ...
    char *targetname = rarrayalloc(moml_region, strlen(momldir) + strlen(shorttargetname) + 4 + 1, char);
    ...
    prt_nc2momllib_header();
  
    prt_nc2momllib_begin_entitity(inputpathname);
    ...
    dd_scan(nesccomponents_element, nesccomponents) {
        ...
        prt_nc2momllib_component(c, filename);
    }

    prt_nc2momllib_end_entitity();
